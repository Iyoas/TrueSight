...
async def explain_cue(payload: CueExplanationRequest):
    ...
    system_message = (
        "You are an assistant inside a visual analytics tool (TrueSight) that helps "
        "users critically inspect how an AI image classifier made its decision.\n"
        "- You NEVER state that an image IS AI-generated or IS real.\n"
        "- You only describe visual evidence that might support or challenge the model's prediction.\n"
        "- You always remind the user that the model can be wrong and that their own judgment matters.\n"
        "- You explain what a given cue (lighting, texture, background, geometry) can tell a human user and what they might want to look at more closely.\n"
        "- Be concise, concrete, and avoid technical jargon."
    )

    user_message = (
        f"The classifier predicted '{prediction}' with {confidence_pct}% confidence.\n"
        f"The cue type is '{cue_type}' (title: '{payload.cue_title}'), "
        f"with a relative focus score of {score_pct}% (0–100 scale).\n\n"
        "Your task:\n"
        "1) Based ONLY on the attached image and, if present, the heatmap for this cue, "
        "describe in 3–4 sentences what stands out visually for this cue in THIS specific image. "
        "Focus on concrete observations (shapes, lighting, textures, background) in the highlighted areas and nearby context.\n\n"
        "2) Explain how these observations could be interpreted both in favour of and against the model's current prediction "
        "(remember: the model can be wrong). Stay neutral; do not say the image is AI or real.\n\n"
        "3) Provide exactly 2 short, practical questions the user can ask themselves while reviewing this cue "
...
async def explain_conclusion(payload: ConclusionSummaryRequest):
    ...
    system_message = (
        ...
        "and a human user arrived at a final conclusion together.\n"
        "- NEVER claim that the image IS AI-generated or IS real; talk only about predictions, judgments, and cues.\n"
        "- Clearly describe what the model predicted and what the human concluded, and how they compare.\n"
        "- Highlight where they agree and where they disagree, and what that implies for trust and uncertainty.\n"
        "- In this tool, the human judgment is leading: the \"final conclusion\" follows the human when they disagree with the model.\n"
        "- Your answer MUST end with a separate final sentence that begins with exactly: 'Final human-led conclusion:' "
        "followed by the human-led verdict in descriptive terms (for example: 'Final human-led conclusion: the human treats this image as likely human-made.').\n"
        "- Use 3–6 sentences total, plain language, no bullet points.\n"
    )

    user_message = (
        "Here is the structured information about the final decision in this case:\n\n"
        f"- Model prediction: {model_label_text} (confidence: {model_conf_pct}).\n"
        f"- Human conclusion: {user_label_text} (self-reported confidence: {user_conf_text}).\n"
        f"- Human-led final conclusion in this tool: {final_label_text}.\n"
        f"- Agreement status: {agreement_text}\n\n"
        "Key cues that influenced this decision:\n"
        f"{cues_block}\n\n"
        "Please write a short narrative paragraph that:\n"
        "- explains how the model arrived at its prediction and how the human arrived at their conclusion,\n"
        "- points out the main similarities and differences between them,\n"
        "- comments briefly on what this means for trust and uncertainty,\n"
        "- and ends with a final sentence that starts with exactly 'Final human-led conclusion:' followed by the human-led verdict "
        "(for example: \"Final human-led conclusion: the human treats this image as likely AI-generated.\")."
    )
